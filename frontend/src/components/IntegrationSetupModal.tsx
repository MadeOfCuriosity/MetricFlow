...
export function IntegrationSetupModal({
  ...
  const handleConfigSubmit = async () => {
    ...
    try {
      ...
      setError(getApiError(err, 'Failed to save configuration'))
    } finally {
  ...
  const handleLoadExternalFields = async () => {
    ...
    try {
      ...
      setError(getApiError(err, 'Failed to fetch fields'))
    } finally {
  ...
  const handleMappingSubmit = async () => {
    ...
    try {
      ...
      setError(getApiError(err, 'Failed to save mappings'))
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      setError(getApiError(err, 'Failed to save schedule'))
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      className="w-full max-w-2xl bg-dark-900 border border-dark-700 rounded-xl shadow-xl"
    >
      {/* Header */}
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-dark-800 border border-dark-600 rounded-lg px-3 py-2 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-brand"
              />
            </div>
            ...
            {isLoadingFields ? (
              ...
              <div className="text-center py-12">
                ...
                <button
                  ...
                  className="mt-3 text-sm text-brand hover:text-brand"
                >
                  Retry
                </button>
              </div>
                ...
                {SCHEDULE_OPTIONS.map((opt) => (
                  ...
                  >
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                      syncSchedule === opt.value ? 'border-brand' : 'border-dark-500'
                    }`}>
                      {syncSchedule === opt.value && (
